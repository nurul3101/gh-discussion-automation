...
async function run() {
  ...
  const closingMessage =
    process.env.CLOSING_MESSAGE || "Closing discussion due to inactivity.";
  ...
  try {
    ...
    const collaborators = collabResponse.data
      ...
      .map((user) => user.login);
    console.log(
      `Found ${collaborators.length} collaborators with write access.`
    );
    ...
    // 2. Fetch top 50 open discussions in the Q&A category using GraphQL
    ...
    const filteredDiscussions = discussions.filter((discussion) => {
      if (!discussion.labels.nodes || discussion.labels.nodes.length === 0)
        return false;
    ...
    for (const discussion of filteredDiscussions) {
      ...
      console.log(
        `Processing discussion #${discussionNumber}: ${discussion.title}`
      );
      ...
      if (
        ...
        try {
          ...
          console.log(
            `Posted closing comment on discussion #${discussionNumber}.`
          );
        } catch (commentError) {
          console.error(`Error posting comment: ${commentError.message}`);
          if (commentError.response)
            console.error(JSON.stringify(commentError.response, null, 2));
          continue;
        }
        ...
        try {
          ...
          if (labelsToRemove.length > 0) {
            ...
            console.log(
              `Removed target labels from discussion #${discussionNumber}.`
            );
          } else {
            console.log(
              `No target labels to remove from discussion #${discussionNumber}.`
            );
          }
        } catch (labelError) {
          console.error(`Error removing labels: ${labelError.message}`);
          if (labelError.errors)
            console.error(JSON.stringify(labelError.errors, null, 2));
        }
        ...
        try {
          ...
          if (closeError.response)
            console.error(JSON.stringify(closeError.response, null, 2));
        }
      } else {
        console.log(
          `Discussion #${discussionNumber} does not qualify for closure.`
        );
      }
    }
  } catch (error) {
